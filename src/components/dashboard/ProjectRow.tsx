...
interface ProjectRowProps {
  ...
  onEdit?: (p: any) => void;
  isAdminView?: boolean;
  // Обновленный тип: теперь принимает и 'PENDING'
      ...
      >
        ...
        <td className="p-6">
          <div className="flex items-center gap-2 justify-center">
            ...
            {isAdminView ? (
              <>
                {/* 1. Если статус PENDING: Показываем кнопки Одобрить/Отклонить */}
                {project.status === 'PENDING' && onStatusUpdate && (
                ...
                {/* 2. Если статус НЕ PENDING: Показываем кнопку "Вернуть в работу" */}
                {project.status !== 'PENDING' && onStatusUpdate && (
                  <button 
                    onClick={(e) => handleAction(e, () => onStatusUpdate(project.id, 'PENDING'))} 
                    className="flex items-center gap-2 px-4 py-3 bg-amber-50 text-amber-600 hover:bg-amber-600 hover:text-white rounded-2xl transition-all animate-in slide-in-from-right-2 duration-300"
                ...
                <button onClick={(e) => handleAction(e, () => onOpenChat?.(project))} className="p-3 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-2xl transition-all">
              ...
              project.status === 'PENDING' && onEdit && (
                <button onClick={(e) => handleAction(e, () => onEdit(project))} className="p-3 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-2xl transition-all">
                  <RefreshCw size={18} />
                </button>
              )
            )}
